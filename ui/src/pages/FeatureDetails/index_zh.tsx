...
const FeatureLineageGraph = (props: { project: string }) => {
  ...
  return !loading ? (
    <Card className="card" title="血缘关系（Lineage）">
    ...
    <div className="page">
      <PageHeader
        breadcrumb={
          <Breadcrumb>
            <Breadcrumb.Item>
              <Link to={`/${project}/features`}>特征列表</Link>
            </Breadcrumb.Item>
            <Breadcrumb.Item>特征详情</Breadcrumb.Item>
          </Breadcrumb>
        }
        extra={[
          <Button
      ...
      >
        <Spin spinning={isLoading} indicator={<LoadingOutlined spin style={{ fontSize: 24 }} />}>
          <Space className="display-flex" direction="vertical" size="middle">
            ...
            <CardDescriptions title="类型（Type）" mapping={TypeMap} descriptions={type} />
            <CardDescriptions title="标签（Tags）" mapping={tagsMap} descriptions={tags} />
